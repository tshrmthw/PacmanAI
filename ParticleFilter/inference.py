...
class ParticleFilter(InferenceModule):
  ...
  def initializeUniformly(self, gameState):
    ...
    for particle in range(0,self.numParticles):
      self.particles.update({legal_positions[int(random.uniform(0,len(legal_positions)))]:1.0})

    self.particles.normalize()
    self.beliefs = self.particles
  ...
  def observe(self, observation, gameState):
    ...
    allPossible = util.Counter()
    for particle_position in self.particles.keys():
      trueDistance = util.manhattanDistance(particle_position,pacmanPosition)
      if emissionModel[trueDistance] > 0:
        allPossible[particle_position] = emissionModel[trueDistance]*self.beliefs[particle_position]
      else:
        zero_weight_counter+=1

    if zero_weight_counter == self.numParticles:
      self.initializeUniformly(gameState)
  ...
  def elapseTime(self, gameState):
    ...
    for particle_position in self.particles.keys():
      if particle_position in self.legalPositions:
        newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState,particle_position))
        for newPos, prob in newPosDist.items():
           newParticles[newPos] += prob*self.beliefs[particle_position]
